...
export function ProjectFileCard({ file, readOnly, onAfterDelete, compactView }: FileCardProps) {
  ...
  if (compactView) {
    return (
      <div className="group flex items-center bg-white shadow-sm mb-2 p-2 border rounded-lg transition-all">
        <div className="mr-3">
          <div className={clsx('flex items-center p-0.5 rounded', bgColor)}>
            <IconComponent className="w-4 h-4 text-white" />
          </div>
        </div>

        <div className="flex-1 mr-2 min-w-0">
          <div className="block font-medium text-gray-700 text-xs truncate">
            {resource.name}
          </div>
        </div>
        ...
        <div className="flex gap-1">
          <a
            className="hover:bg-gray-100 p-1 rounded-md text-gray-500 hover:text-gray-900 transition-colors"
            title={t.download}
          ...
          {!readOnly && (
            <button
              ...
              className="hover:bg-red-50 p-1 rounded-md text-gray-500 hover:text-red-600 transition-colors"
              title={t.delete}
              type="button"
            >
